...
let paused = false;
let gameOver = false;
let levelFinished = false; // NOVA VARIÁVEL PARA EVITAR LOOP
let imagesLoaded = 0;
let totalImages = 0;
...
function setupEventListeners() {
    document.addEventListener('keydown', e => {
      if (gameOver || levelFinished) return; // Bloqueia se acabou
      keys[e.key] = true;
...
// ==================== FUNÇÕES DE EXIBIÇÃO DE TELA (CORRIGIDAS) ====================

function showGameOver() {
  if(gameOver) return; // Evita chamada duplicada
  gameOver = true;
...
function showLevelComplete() {
    if(levelFinished) return; // Evita chamada duplicada
    levelFinished = true;
    paused = true; // PAUSA O JOGO IMEDIATAMENTE
...
async function goToMainMenu() {
    if (player.score > 0) {
        // Tenta salvar, mas não bloqueia a saída se demorar
        enviarPontuacaoParaBanco(player.score).catch(e => console.log("Erro ao salvar ao sair:", e));
    }
...
// ==================== LÓGICA DO JOGO (COM TRAVAMENTO) ====================
function checkFruitCollection() {
    if(levelFinished) return; // Se já acabou, não checa mais
    ...
    if (allCollected) {
      showLevelComplete(); // Aqui ele vai travar o loop
    }
}
...
function gameLoop() {
    ...
    // Se o jogo não estiver pausado, nem game over, nem fase terminada
    ...
    const data = await response.json();
      ...
      if(isRecord && elementIdForRecordMsg) {
           ...
           if(el) {
               ...
               console.log("NOVO RECORDE EXIBIDO NA TELA!");
           }
       } 
       ...
       if(elementIdForSaveMsg && document.getElementById(elementIdForSaveMsg)) {
          document.getElementById(elementIdForSaveMsg).textContent = "Erro ao salvar pontuação.";
